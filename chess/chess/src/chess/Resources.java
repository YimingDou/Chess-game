...
public class Resources
{
	...
	public final static int NUM_PIECES_PER_PLAYER = 16;
    ...
    /** The types of each of the (16) pieces per player */
	public final static PieceType[] PIECE_INSTANCE_TYPES = new PieceType[]	
	{
		PieceType.KING, PieceType.QUEEN, PieceType.ROOK, PieceType.ROOK, PieceType.BISHOP, PieceType.BISHOP, PieceType.KNIGHT, PieceType.KNIGHT,
	...
	public static void loadImages()
	{
		...
		for (int i = 0; i < PIECE_NAMES.length; i++)
		{
			...
			File whiteImageFile = new File("images" + File.separator + "white_" + filePieceName + ".png");
			File blackImageFile = new File("images" + File.separator + "black_" + filePieceName + ".png");
     ...
     */
	public static boolean canMove(PieceType type, BoardLocation origin, BoardLocation destination, BoardInterface board, Player player)
	{
     ...
     * result
     */
	public static MoveState getMove(PieceType type, BoardLocation origin, BoardLocation destination, BoardInterface board, Player player)
	{
		...
		{
            // ...and it belongs to the player who is making the move
			if (board.isFriendly(destination, player))
			{
                Resources.trace("...EXITING Resources.getMove(...) early with a value of " + MoveState.NOT_MOVABLE);
                // ...then return the MoveState that represents that the move cannot be made.
                return MoveState.NOT_MOVABLE;
			}
		}
		
		MoveState successfulMoveState = board.isEnemy(destination, player) ? MoveState.CAPTURABLE : MoveState.MOVABLE;
     ...
     */
	public static boolean isPawnMovable(BoardLocation origin, BoardLocation destination, BoardInterface board, Player currentPlayer)
	{
        // Calculate some intermediate values
		int differenceX = getDifferenceX(origin, destination);
		int differenceY = getDifferenceY(origin, destination);
		int forwardDifferenceY = (currentPlayer == Player.WHITE ? differenceY : - differenceY);
		int absoluteDifferenceX = getAbsoluteDifferenceX(origin, destination);
		boolean destinationContainsEnemy = board.isEnemy(destination, currentPlayer);
		boolean onStartingRow = (currentPlayer == Player.WHITE ? origin.getY() == 1 : origin.getY() == BOARD_SIZE - 1 - 1);
		
		
        // If the move is one row forward...
		if (forwardDifferenceY == 1)
		{
			if ((absoluteDifferenceX == 0 && !destinationContainsEnemy) || (absoluteDifferenceX == 1 && destinationContainsEnemy))
			{
				return true;
			}
		}
        // If the move is two rows forward...
		if (forwardDifferenceY == 2)
		{
			if (onStartingRow && absoluteDifferenceX == 0 && !destinationContainsEnemy)
			{
				return onStartingRow;
			}
		}
		return false;
	}
	...
	public static BoardLocation getStartingLocation(Player owner, int pieceIndex)
	{
		...
		PieceType type = PIECE_INSTANCE_TYPES[pieceIndex];
		
        // If the piece is a pawn...
		if (type == Resources.PieceType.PAWN)
		{
            // ... put it in the first row
			rank = 1;
            // ... and place it horizontally according to its index.
			relativePositionX = pieceIndex - BOARD_SIZE;
		}
        // ...otherwise rely on hardcoded values.
		else
		{
			switch (pieceIndex)
			{
				case 0 : relativePositionX = 4; break;
				case 1 : relativePositionX = 3; break;
				case 2 : relativePositionX = 0; break;
				case 3 : relativePositionX = 7; break;
				case 4 : relativePositionX = 2; break;
				case 5 : relativePositionX = 5; break;
				case 6 : relativePositionX = 1; break;
				case 7 : relativePositionX = 6; break;
				default : relativePositionX = -1; break;
			}
		}
		
        // Rotate the coordinates if the piece belongs to the black player.
		int x = (owner == Player.WHITE ? relativePositionX : BOARD_SIZE - (relativePositionX + 1));
		int y = (owner == Player.WHITE ? rank : BOARD_SIZE - (rank + 1));
		
        BoardLocation returnLocation = new BoardLocation(x, y);
        Resources.trace("...EXITING Resources.hasClearPathTo(" + owner + ", " + pieceIndex + ") with a value of " + returnLocation);
